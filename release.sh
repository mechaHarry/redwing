...
PROJECT_PATH="${ROOT_DIR}/redwing.xcodeproj"
VERSION_CONFIG="${ROOT_DIR}/Redwing/Config/Version.xcconfig"
DERIVED_DATA_PATH="${DERIVED_DATA_PATH:-/private/tmp/redwing-release}"
...
usage() {
  cat <<USAGE
Usage: ./release.sh [--dry-run|--package-only]

Reads REDWING_SEMVER from Redwing/Config/Version.xcconfig, builds a Release
app bundle, creates a versioned zip asset plus SHA-256 checksum, creates a
signed git tag named v<REDWING_SEMVER>, pushes it, creates a GitHub release
with generated release notes, uploads the assets, then publishes the release.
...
Environment:
  ...
  DERIVED_DATA_PATH     Xcode derived data path. Default: /private/tmp/redwing-release
  CODE_SIGNING_ALLOWED  Passed to xcodebuild. Default: NO
USAGE
}
...
read_version() {
  if [[ ! -f "${VERSION_CONFIG}" ]]; then
    echo "error: ${VERSION_CONFIG} does not exist" >&2
    exit 1
  fi

  VERSION="$(
    sed -nE 's/^[[:space:]]*REDWING_SEMVER[[:space:]]*=[[:space:]]*([^[:space:]]+)[[:space:]]*$/\1/p' "${VERSION_CONFIG}" | tail -n 1
  )"

  if [[ ! "${VERSION}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "error: REDWING_SEMVER must be MAJOR.MINOR.PATCH without a leading v" >&2
    exit 1
  fi
...
package_app() {
  local build_app_path
  local plist_version
  local plist_build

  echo "Building ${APP_NAME} ${VERSION}..."
  xcodebuild build -project "${PROJECT_PATH}" -scheme "${SCHEME}" -configuration Release -destination 'platform=macOS' -derivedDataPath "${DERIVED_DATA_PATH}" CODE_SIGNING_ALLOWED="${CODE_SIGNING_ALLOWED}"

  build_app_path="${DERIVED_DATA_PATH}/Build/Products/Release/${APP_NAME}.app"
  if [[ ! -d "${build_app_path}" ]]; then
    echo "error: ${build_app_path} does not exist after build" >&2
    exit 1
  fi

  rm -rf "${APP_PATH}"
  mkdir -p "${DIST_DIR}"
  ditto "${build_app_path}" "${APP_PATH}"
  ...
  if [[ "${plist_version}" != "${VERSION}" || "${plist_build}" != "${VERSION}" ]]; then
    echo "error: packaged Info.plist version/build ${plist_version}/${plist_build} does not match REDWING_SEMVER ${VERSION}" >&2
    exit 1
  fi
...
ensure_release_does_not_exist() {
  ...
  existing_release_json="${TMPDIR:-/tmp}/redwing-existing-release-${TAG_NAME}.json"
  if api_curl "${API_ROOT}/repos/${OWNER}/${REPO}/releases/tags/${TAG_NAME}" >"${existing_release_json}" 2>/dev/null; then
    existing_release_url="$(python3 -c 'import json, sys; print(json.load(sys.stdin)["html_url"])' <"${existing_release_json}")"
    echo "error: release ${TAG_NAME} already exists: ${existing_release_url}" >&2
    echo "Bump REDWING_SEMVER and release a new tag." >&2
    exit 1
  fi
}
...
if [[ ! -x /usr/libexec/PlistBuddy ]]; then
  echo "error: required command '/usr/libexec/PlistBuddy' was not found" >&2
  exit 1
fi
...
if [[ "${DRY_RUN}" -eq 0 && "${PACKAGE_ONLY}" -eq 0 ]]; then
